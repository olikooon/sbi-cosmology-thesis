...
Это часть подготовки к дипломной работе на тему **Simulation-Based Inference (SBI) для космологической параметрической инференции на симуляциях CAMELS**. Перед разработкой собственной методологии было необходимо запустить и понять один из ключевых SOTA-методов области — от загрузки данных до получения итогового posterior, а не только прочитать статью.

Воспроизведение решает три задачи:

1. **Понимание архитектуры "изнутри"** — как устроен полный pipeline likelihood-free inference: нейронная компрессия summary statistics → ensemble density estimation (pyDELFI) → MCMC-сэмплирование posterior.
2. **Валидация собственной реализации** — сравнение полученных констрейнтов на реальных данных DES SV с published результатами статьи.
3. **Инфраструктурная база** — понятая архитектура (компрессор + density estimator + coverage-валидация) переносится на основную часть дипломной работы, где та же логика применяется к симуляциям CAMELS с использованием современной библиотеки `sbi`.
